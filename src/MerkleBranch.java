...
public class MerkleBranch {
    ...
    public MerkleBranch(InputStream inStream) throws EOFException, IOException, VerificationException {
        ...
        if (txCount < 1 || txCount > Parameters.MAX_BLOCK_SIZE/60)
    ...
    private void buildBranch(int height, int levelPos, int[][]nodePath, int treePos, List<byte[]>merkleTree) {
        ...
        if (height > 0) {
            ...
            if (nextNode == nextLevelPos) {
                ...
                Utils.setBitLE(nodeFlags, bitsUsed+1);
                bitsUsed += 2;
                buildBranch(height-1, nextLevelPos+1, nodePath, treePos-levelWidth, merkleTree);
            } else {
    ...
    private byte[] parseBranch(int height, int pos, List<Sha256Hash> matchedHashes)
        ...
        //
        return Utils.reverseBytes(Utils.doubleDigestTwoBuffers(Utils.reverseBytes(left), 0, 32,
